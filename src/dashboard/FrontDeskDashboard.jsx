...
export default function FrontDeskDashboard() {
  ...
  .order('start_time', { ascending: true });
      ...
      reservationData?.forEach(item => {
        ...
        if (item.customer_status === 'active' || item.customer_status === 'extended' || item.customer_status === 'checked-in') {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        setSelectedCustomer({
          id: found.id,
          customer: found.account?.name || 'Unknown Customer',
          table: found.table?.table_number || `T-${found.table_id}`,
          phone: found.account?.phone || 'N/A',
          status: found.customer_status || found.status,
          startTime: found.start_time,
          duration: `${found.duration}h`,
          amount: found.total_bill ? `₱${found.total_bill}` : '₱0'
        });
      } else {
        ...
        >
          ...
          <div className="relative p-6">
            <div className={`flex items-center justify-between mb-4 ${activeTab === 'payment' ? 'text-white' : 'text-gray-700'}`}>
              <DollarSign size={40} className="transition-transform group-hover:rotate-12" />
              <TrendingUp size={20} className="opacity-60" />
            </div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-2">
                        <button
                          ...
                          className="px-4 py-2 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-lg hover:shadow-lg transform hover:scale-105 transition-all text-xs font-semibold"
                        >
                          Extend
                        </button>
                        <button
                          onClick={() => handleFinalizePayment(session.id)}
                          className="px-4 py-2 bg-gradient-to-r from-emerald-500 to-green-600 text-white rounded-lg hover:shadow-lg transform hover:scale-105 transition-all text-xs font-semibold"
                        >
                          Payment
                        </button>
                      </div>
                    ...
                    <td className="px-6 py-4">
                      {reservation.status !== 'checked-in' ? (
                        <button
                          ...
                          className="px-4 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg hover:shadow-lg transform hover:scale-105 transition-all text-xs font-semibold flex items-center gap-2"
                        >
                          <CheckCircle size={14} />
                          Check In
                        </button>
                      ) : (
                        <span className="text-teal-600 text-xs font-bold flex items-center gap-2 px-4 py-2 bg-teal-50 rounded-lg border border-teal-200">
                          <CheckCircle size={14} />
                          CHECKED IN
                        </span>
                      )}
